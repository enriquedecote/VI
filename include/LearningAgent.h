...
class LearningAgent {
	...
	public:
		...
		*/
		...
		/**
		* TODO: bisogna trovare il modo di decidere dal riga di comando quale politica
		* di exploration utilizzare e di conseguenza farsi passare anche il valore
		* di temperatura
		* Select the action to be performed at the next step.
		* @param rIsExplorative  true if the action selected has been extracted
		*                        randomly according to the Boltzmann exploration 
		*                        strategy
		* @return the value of the action selected
		*/
		//virtual Variable* SelectActionBoltzmann(bool& rIsExplorative);

		virtual void UpdateInitialValues() = 0;
		...
		*/
		virtual void Update(float payoff, VariablesInfo* stateDescription) = 0;
	...
	protected:
		...
		*                     the old value
		*/
		virtual void UpdateValue(float* rNewValue, float payoff, float bestValue, float learningRate = 0.0) = 0;
		...
		*/
		...
		/**
		* The learning rate is the step size in the mobile mean formula
		* used to update the Q-values.
		*/
		float mLearningRate;
		
		/**
		* The initial value for the exploration factor.
		*/
		float mInitialLearningRate;
		
		/**
		* The decreasing rate used to decrement the learning rate of the
		* algorithm. The formula used to update the learning rate is:
		*    new_val = init_val/(1.0+number_of_visits*dec_rate)
		* where number_of_visits is the number of visits to the specific
		* pair (state,action).
		*/
		float mDecreasingRate;
		
		/**
		* The exploration factor is the probability to select an explorative
		* action during the action selection phase.
		*/
		...
		* The initial value for the exploration factor.
		*/
		float mInitialExplorationFactor;
		
		/**
		* The rate used to decrement the exploration factor at each time
		* step. The formula used to update the exploration factor is the
		* following:
		*    new_val = init_val - dec_rate*steps
		* It is a linear decrement until new_val is less or equal to 0.
		*/
		float mDecRateExplorationFactor;
		
		/**
		* TODO: da definire in modo più preciso, in particolare la legge
		* con cui farla decrescere
		*/
		float mTemperature;
		...
		*/
		...
		/**
		* Contains the value of the selected explorative action, if any.
		*/
		Variable* mExplorativeAction;
		...
		*/
		map<float, map<Variable*, float> > mJointQTable;
		
		/**
		* The number of visits for each pair (state,action) where the
		* state is the joint action.
		* < row (state); < action,payoff > >
		*/
		map<float, map<Variable* , unsigned > > mJointExperience;
		
		/**
		* If the action currently selected is explorative or not.
		*/
		bool mIsCurrentActionExplorative;
